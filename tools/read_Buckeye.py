...
def check_arguments():
    parser = argparse.ArgumentParser(prog="read_Buckeye", description="This program creates a MySQL database containing the data from the Buckeye .words files so that they are searchable by Coquery.""")
    
    parser.add_argument("db_name", help="name of the MySQL database to be used", type=str)
    parser.add_argument("path", help="location of the Buckeye .word files ", type=str)
...
def get_file_list(path, file_dict={}):
    ...
    for source_path, folders, files in os.walk(path):
        for current_file in files:
            if fnmatch.fnmatch(current_file, "*.words"):
