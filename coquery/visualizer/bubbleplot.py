    ...
    def draw(self):
        ...
        def plot_facet(data, color):
            ...
            def draw_circle(k):
                ...
                self.circles.append((self.pos[k], rad,
                                     "{}: {}".format(label, freq)))
            ...
            if len(self._groupby) == 2:
                df_freq.sort([self._groupby[-1], "Freq"],
            ...
            for i in range(len(df_freq.index)):
                if i == 0:
                    ...
                    self.pos[1] = (self.pos[0][0] +
                                   df_freq.iloc[0]["r"] +
                                   df_freq.iloc[1]["r"], self.pos[0][1])
                else:
                    self.pos[i] = get_position(i, a, n)
                    while get_intersections(i, completed, n) and a < i:
                        a = a + 1
                        # Intersections not only trigger an anchor bubble
                        # change, they also indicate that a closed ring of
                        # bubbles has been completed. All bubbles within the
                        # closed ring do not have to be considered for
                        # collision detections, so keeping track of the
                        # completed bubbles speeds up the # algorithm:
