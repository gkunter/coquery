...
"""
coq_install_beep.py is part of Coquery.
...
class BuilderClass(BaseCorpusBuilder):
    ...
    def validate_files(l):
        ...
        if len(l) == 0:
            raise RuntimeError("<p>No dictionary file could be found in the selected directory. The file name of the dictionary file has to start with the sequence <code>beep-</code>.</p> ")
    ...
    def build_load_files(self):
        ...
        for i, current_line in enumerate(content):
            ...
            if current_line and not current_line.startswith ("#"):
                ...
                if len(fields) == 1:
                    word, _, transcript = current_line.partition(" ")
                else:
                    word = fields[0]
                    transcript = fields[-1]
                ipa = transpose.arpa_to_ipa(transcript)
    ...
    @staticmethod
    def get_title():
        return "British English Example Pronciation dictionary"

    @staticmethod
    def get_url():
        return "ftp://svr-ftp.eng.cam.ac.uk/pub/comp.speech/dictionaries/beep.tar.gz"
    ...
    @staticmethod
    def get_license():
        return "BEEP is available under the terms of a research-only license."
    
    @staticmethod
    def get_description():
        return ["The British English Example Prounciation dictionary (BEEP) is a dictionary containing approximately 250.000 English word-forms and their phonemic transcriptions. It was compiled in 1996 by Tony Robinson."]
