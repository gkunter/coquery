...
class BuilderClass(BaseCorpusBuilder):
    ...
    def process_text(self, text):
        for row in text.splitlines():
            try:
                self._value_corpus_word, self._value_corpus_pos, self._value_corpus_lemma = [x.strip() for x in row.split("\t")]
            except ValueError:
                pass
            else:
                self._value_corpus_word = self._replace_encoding_errors(self._value_corpus_word)
                self._value_corpus_lemma = self._replace_encoding_errors(self._value_corpus_lemma)
                new_sentence = False
                
                if self._value_corpus_pos == "CD":
                    self._value_corpus_lemma = self._value_corpus_word
                if self._value_corpus_pos in string.punctuation or self._value_corpus_pos == "''":
                    self._value_corpus_pos = "PUNCT"
                if self._value_corpus_pos == "SENT":
                    new_sentence = True
                    self._value_corpus_pos = "PUNCT"
                    
                if self._value_corpus_word and self._value_corpus_lemma:
                    self.add_token_to_corpus(
                        {self.corpus_word: self._value_corpus_word,
                         self.corpus_lemma: self._value_corpus_lemma,
