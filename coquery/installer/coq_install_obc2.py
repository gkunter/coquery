...
class BuilderClass(TEICorpusBuilder):
    ...
    word_columns = [
        ...
        Column(word_label, "VARCHAR(25) NOT NULL"),
        Column(word_pos, "VARCHAR(4) NOT NULL")]
    ...
    source_columns = [
        ...
        Column(source_label, "VARCHAR(13) NOT NULL"),
        Column(source_type, "ENUM('trialAccount','supplementaryMaterial') NOT NULL"),
    ...
    corpus_columns = [
        ...
        Column(corpus_sentence, "VARCHAR(12) NOT NULL")]
    ...
    @staticmethod
    def get_url():
        return "http://fedora.clarin-d.uni-saarland.de/oldbailey/downloads.html"

    @staticmethod
    def get_license():
        return "The <i>Old Bailey Corpus</i> is licensed under a Creative Commons Attribution-NonCommercial-ShareAlike license (<a href='http://creativecommons.org/licenses/by-nc-sa/4.0/'>CC BY-NC-SA 4.0</a>)."
    ...
    @classmethod
    def _split_words(cls, s):
        elements = [x for x in s.split() if x]
        if elements:
            words, _, tags = zip(*(w.rpartition("_") for w in elements))
