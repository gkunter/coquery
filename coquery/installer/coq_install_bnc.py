...
class BuilderClass(BaseCorpusBuilder):
    ...
    def __init__(self, gui=False, *args):
            ...
            [Identifier(self.word_id, "MEDIUMINT(7) UNSIGNED NOT NULL"),
             ...
             Column(self.word_pos,
                    ("ENUM('AJ0','AJ0-AV0','AJ0-NN1','AJ0-VVD','AJ0-VVG',"
                     ...
                     "'AVP-PRP','AVQ','AVQ-CJS','CJC','CJS','CJS-AVQ',"
                     "'CJS-PRP','CJT','CJT-DT0','CRD','CRD-PNI','DPS',"
                     "'DT0','DT0-CJT','DTQ','EX0','ITJ','NN0','NN1',"
                     "'NN1-AJ0','NN1-NP0','NN1-VVB','NN1-VVG','NN2',"
                     "'NN2-VVZ','NP0','NP0-NN1','ORD','PNI','PNI-CRD','PNP',"
                     "'PNQ','PNX','POS','PRF','PRP','PRP-AVP','PRP-CJS',"
                     "'PUNCT','TO0','UNC','VBB','VBD','VBG','VBI','VBN',"
                     "'VBZ','VDB','VDD','VDG','VDI','VDN','VDZ','VHB','VHD',"
                     "'VHG','VHI','VHN','VHZ','VM0','VVB','VVB-NN1','VVD',"
                     "'VVD-AJ0','VVD-VVN','VVG','VVG-AJ0','VVG-NN1','VVI',"
                     "'VVN','VVN-AJ0','VVN-VVD','VVZ','VVZ-NN2','XX0','ZZ0') "
                     "NOT NULL")),
             Column(self.word_lemma_pos,
                    ("ENUM('ADJ','ADV','ART','CONJ','INTERJ','PREP','PRON',"
                     "'PUNCT','SUBST','UNC','VERB') NOT NULL")),
            ...
            [Identifier(self.speaker_id, "SMALLINT(4) UNSIGNED NOT NULL"),
             ...
             Column(self.speaker_age,
                    ("ENUM('','-82+','0','1','10','10+','11','12','13','13+',"
                     ...
                     "'28','29','3','3+','30','30+','31','32','33','34','35',"
                     "'35+','36','37','38','39','4','40','40+','41','42',"
                     "'43','44','45','45+','46','46+','47','48','48+','49',"
                     "'5','50','50+','51','52','53','54','55','55+','56',"
                     "'57','58','59','6','60','60+','61','62','63','64','65',"
                     "'65+','66','67','68','69','7','70','70+','71','72',"
                     "'73','74','75','75+','76','77','78','79','8','80',"
                     "'80+','81','82','84','86','87','89','9','92','93',"
                     "'95') NOT NULL")),
        ...
        self.create_table_description(
            ...
            [Identifier(self.source_id, "SMALLINT(4) UNSIGNED NOT NULL"),
             ...
             Column(self.source_genre,
                    ("ENUM('ACPROSE','CONVRSN','FICTION','NEWS','NONAC',"
                     "'OTHERPUB','OTHERSP','UNPUB') NOT NULL")),
             Column(self.source_class,
                    ("ENUM('S brdcast discussn','S brdcast documentary',"
                     "'S brdcast news','S classroom','S consult','S conv',"
                     "'S courtroom','S demonstratn','S interview',"
                     "'S interview oral history','S lect commerce',"
                     "'S lect humanities arts','S lect nat science',"
                     "'S lect polit law edu','S lect soc science',"
                     "'S meeting','S parliament','S pub debate','S sermon',"
                     "'S speech scripted','S speech unscripted',"
                     "'S sportslive','S tutorial','S unclassified',"
                     "'W ac:humanities arts','W ac:medicine',"
                     "'W ac:nat science','W ac:polit law edu',"
                     "'W ac:soc science','W ac:tech engin','W admin',"
                     "'W advert','W biography','W commerce','W email',"
                     "'W essay school','W essay univ','W fict drama',"
                     "'W fict poetry','W fict prose','W hansard',"
                     "'W institut doc','W instructional',"
                     "'W letters personal','W letters prof','W misc',"
                     "'W news script','W newsp brdsht nat: arts',"
                     ...
                     "'W newsp brdsht nat: sports','W newsp other: arts',"
                     "'W newsp other: commerce','W newsp other: report',"
                     "'W newsp other: science','W newsp other: social',"
                     "'W newsp other: sports','W newsp tabloid',"
                     "'W nonAc: humanities arts','W nonAc: medicine',"
                     "'W nonAc: nat science','W nonAc: polit law edu',"
                     "'W nonAc: soc science','W nonAc: tech engin',"
                     "'W pop lore','W religion') NOT NULL")),
             Column(self.source_year,
                    ("ENUM('0000','1947','1960','1961','1962','1963','1964',"
    ...
    def xml_preprocess_tag(self, element):
        ...
        # <s> is a sentence:
            ...
            self._sentence_id = self.table(
                self.sentence_table).get_or_insert(
                    {self.sentence_label: sentence})
    ...
    def xml_get_meta_info(self, root):
        ...
        if participant_desc is not None:
            for person in participant_desc.findall("person"):
                tup = self.get_speaker_data(person)
                speaker_label, speaker_age, speaker_sex = tup
                self._speaker_dict[speaker_label] = (
                    self.table(self.speaker_table)
                        .get_or_insert({self.speaker_label: speaker_label,
        ...
        d = {self.source_xmlname: self._value_source_xmlname,
    ...
    def xml_get_body(self, root):
        ...
        if body is None:
            logging.warning(
                "Neither <wtext> nor <stext> found in file, not processed.")
        return body
    ...
    def get_description():
        return [
            ("The British National Corpus (BNC) is a 100 million word "
             ...
             "cross-section of British English from the later part of the "
             "20th century, both spoken and written."),
            ("The written part of the BNC (90%) includes, for example, "
             "extracts from regional and national newspapers, specialist "
             "periodicals and journals for all ages and interests, academic "
             "books and popular fiction, published and unpublished letters "
             "and memoranda, school and university essays, among many other "
             "kinds of text."),
            ("The spoken part (10%) consists of orthographic transcriptions "
    ...
    @staticmethod
    def get_license():
        return ("The BNC is available under the terms of the "
                "<a href='http://www.natcorp.ox.ac.uk/docs/licence.html'>"
                "BNC User Licence</a>.")

    @staticmethod
    def get_references():
        return [("<i>The British National Corpus</i>, version 3 (BNC XML "
                 "Edition). 2007. Distributed by Oxford University Computing "
