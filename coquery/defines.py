...
QUERY_MODES = {"TOKEN": QUERY_MODE_TOKENS,
...
# user interface
SUMMARY_MODES = [QUERY_MODE_TOKENS,
                 QUERY_MODE_TYPES,
...
<p><b>The file that you selected does not appear to contain valid meta data.
</b></p>
...
msg_nltk_tagger = """Part-of-speech tags were assigned using <code>{tagger}
</code>, the recommended tagger from the Natural Language Toolkit (NLTK)
version {version}. NLTK used data from WordNet for lemmatization."""

msg_nltk_tagger_fallback = """Part-of-speech tags were assigned using the
default tagger from the Natural Language Toolkit (NLTK) version {version}.
NLTK used data from WordNet for lemmatization."""
...
<p><code>\t{}</code></p>
<p>Please refer to the <a href="http://coquery.org/doc/">Coquery documentation
</a> for instructions on how to install the required modules.</p>
"""
msg_missing_module = """
<p><b>The optional Python module '<code>{name}</code>' could not be loaded.</b>
</p>
...
<p>To help us, include the following text in your message:<br><br>
...
<br><br><code>{}</code></p>
<p>You can abort Coquery now, and try to fix the problem with the configuration
file.</p>
...
authors about it so that the problem may be fixed in a future version.</p>"""
msg_query_running = """
<p><b>The last query is still running.</b></p>
<p>If you interrupt it, the results that have been retrieved so far will be
discarded.</p>
...
encoding <code>{encoding}</code>. Choose a different encoding from the list.
</p>"""
msg_csv_file_error = """
...
one of the supported character encodings. Unicode/UTF-8 is strongly
recommended.</p>"""
msg_filename_error = """
