...
class LogfileViewer(QtWidgets.QDialog):
    def __init__(self, parent=None):
        ...
        for x in button_map:
            button = getattr(self.ui, button_map[x])
            button.setChecked(x in options.cfg.show_log_messages)
            button.stateChanged.connect(self.changeFilters)
    ...
    def changeFilters(self):
        ...
        for filt, checkbox in button_map.items():
            button = getattr(self.ui, checkbox)
            if button.isChecked():
